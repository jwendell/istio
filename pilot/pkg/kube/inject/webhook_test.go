...
func TestInjectRequired(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			config: &Config{
				Policy:              InjectionPolicyEnabled,
				NeverInjectSelector: *parseToLabelSelector(t, "foo"),
			},
			podSpec: podSpec,
			meta: &metav1.ObjectMeta{
		...
		},
		{
			config: &Config{
				Policy:               InjectionPolicyDisabled,
				AlwaysInjectSelector: *parseToLabelSelector(t, "foo"),
			},
			podSpec: podSpec,
			meta: &metav1.ObjectMeta{
		...
		},
		{
			config: &Config{
				Policy:               InjectionPolicyDisabled,
				AlwaysInjectSelector: *parseToLabelSelector(t, "always"),
				NeverInjectSelector:  *parseToLabelSelector(t, "never"),
			},
			podSpec: podSpec,
			meta: &metav1.ObjectMeta{
